...
import {SubscriptionResponseStatus, PaymentHistoryResponseStatus} from '@/api/types'
...
export default function SubscriptionStatusBadge({status}: SubscriptionStatusBadgeProps) {
	switch (status) {
		...
		case SubscriptionResponseStatus.EXPIRED:
			return (
				<Badge variant="warning">Истекла</Badge>
			)
		case SubscriptionResponseStatus.PENDING_PAYMENT:
			return (
				<Badge variant="neutral">Ожидает оплаты</Badge>
			)
		default:
			return (
